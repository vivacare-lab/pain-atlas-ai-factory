...
`Content Object.lifecycle.current_state`는 `state-machine.md`의 Canonical Content State만 저장한다.

Workflow 실행 단계는 `Content Object.workflow.workflow_stage`에 저장한다.

```text
State Machine
= allowed semantic content transitions

Service Flow
= workflow execution stages

Content Object
= current semantic state + current outputs + references
```
